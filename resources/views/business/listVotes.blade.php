...
@section('extra-css')
    <style>
        ...
        .vt-filter-select-icon:disabled {
            opacity: .5;
            cursor: not-allowed;
        }
        ...
        .vt-votes-table tbody tr:last-child td {
            border-bottom: none;
        }

        .vt-votes-table tbody tr:hover td {
            background: #fafbfc;
        }
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#filter-campagne').on('change', function() {
                ...
                $etape.val('').prop('disabled', !campagneId);
            ...
            /* -------------------------------------------------------
               Tous les autres filtres → déclenchement automatique
               ------------------------------------------------------- */
            $('#filter-etape, #filter-status').on('change', function() {
                loadVotes();
            });

            let dateTimeout = null;
            $('#filter-date-debut, #filter-date-fin').on('change input', function() {
                clearTimeout(dateTimeout);
                dateTimeout = setTimeout(loadVotes, 400); // petit délai pour les dates
            });
            ...
            //     $('#filter-date-debut, #filter-date-fin').val('');
            //     $('#filter-status').val('');
            //     loadVotes();
            // });
            $('#btn-reset-filters').on('click', function() {
                // Réinitialiser les valeurs sans déclencher les listeners
                $('#filter-campagne').val('');
                $('#filter-etape').val('').prop('disabled', true).find('option').not(':first').hide();
                $('#filter-date-debut, #filter-date-fin').val('');
                $('#filter-status').val('');

                // Réinitialiser les stats
                $('#btn-total-votes').text('0');
                $('#btn-total-montant').text('0 FCFA');
                $('#results-label').text('Résultats filtrés');

                // Afficher tableau vide directement, sans appel AJAX
                $('#votes-table-body').html(

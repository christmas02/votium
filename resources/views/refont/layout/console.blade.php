...
<head>
    ...
    <title>@yield('title', 'Votium') — Administration</title>
    ...
    {{-- ================================================
         VOTIUM REFONT — Design System (identique espace promoteur)
         ================================================ --}}
    <style>
        /* ---- Variables ---- */
        :root {
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .vt-navbar-inner {
            ...
            height: 100%; padding: 0 28px; gap: 20px;
        }
        .vt-navbar-brand { display: flex; align-items: center; gap: 10px; flex-shrink: 0; }
        .vt-logo-circle {
            width: 34px; height: 34px; background: var(--vt-orange);
        ...
        .vt-nav-links li a {
            ...
            font-size: 12.5px; font-weight: 500; transition: all .15s; white-space: nowrap;
        }
        ...
        .vt-promoteur-btn {
            ...
            background: var(--vt-orange); border: none; border-radius: 50px;
            padding: 5px 14px 5px 8px; color: #fff; font-size: 12.5px; font-weight: 600;
        ...
        .vt-deco-line {
            width: 32px; height: 3px; background: var(--vt-orange);
            border-radius: 2px; margin: 4px auto 16px;
        }

        /* ---- Page body ---- */
        .vt-page-body { padding: 0 28px 32px; }

        /* ---- Deux colonnes ---- */
        .vt-two-col { display: flex; align-items: flex-start; gap: 18px; }
        .vt-filter-col { width: 248px; flex-shrink: 0; }
        .vt-filter-card {
            background: var(--vt-card-bg); border-radius: var(--vt-radius);
            padding: 20px 18px; box-shadow: var(--vt-shadow);
        }
        .vt-filter-header {
            display: flex; align-items: center;
        ...
        .vt-badge-live::before {
            content: ''; width: 6px; height: 6px; background: var(--vt-green);
            border-radius: 50%; animation: vt-pulse 1.5s infinite;
        }
        ...
        .vt-filter-select {
            ...
            appearance: none; font-size: 12.5px; color: var(--vt-text-main);
        ...
        .vt-filter-info {
            background: var(--vt-orange-light); border: 1px solid var(--vt-orange-border);
            border-radius: var(--vt-radius-sm); padding: 12px 13px;
            font-size: 12px; color: #92400e; line-height: 1.9;
        }
        .vt-filter-info strong { color: #c2410c; }

        /* ---- Colonne principale ---- */
        .vt-main-col { flex: 1; min-width: 0; display: flex; flex-direction: column; gap: 14px; }

        /* ---- Composants réutilisables ---- */
        .vt-card { background: var(--vt-card-bg); border-radius: var(--vt-radius); box-shadow: var(--vt-shadow); }
        ...
        .vt-stats-row {
            ...
            gap: 1px; background: var(--vt-border);
            border-radius: 10px; overflow: hidden;
        }
        .vt-stat-item { background: #fff; padding: 14px 16px; }
        ...
        .vt-btn-primary {
            ...
            border-radius: 50px; padding: 9px 20px; font-size: 13px; font-weight: 600;
        ...
        .vt-chart-card {
            background: var(--vt-card-bg); border-radius: var(--vt-radius);
            padding: 18px 18px 10px; box-shadow: var(--vt-shadow); height: 100%;
        }
        .vt-chart-header {
            display: flex; align-items: flex-start;
            justify-content: space-between; margin-bottom: 4px;
        }
        .vt-chart-title { font-size: 13.5px; font-weight: 600; color: var(--vt-text-main); margin: 0; }
        .vt-chart-badge {
            font-size: 10.5px; font-weight: 600; color: var(--vt-text-muted);
            background: var(--vt-page-bg); padding: 2px 8px; border-radius: 4px; flex-shrink: 0;
        }
        ...
        .vt-table thead th {
            padding: 10px 16px; text-transform: uppercase; font-size: 10.5px;
            letter-spacing: .5px; color: var(--vt-text-muted);
            border-bottom: 1px solid var(--vt-border); font-weight: 600; white-space: nowrap;
        }
        .vt-table tbody td {
            padding: 12px 16px; border-bottom: 1px solid var(--vt-border); color: var(--vt-text-main);
        }
        .vt-table tbody tr:last-child td { border-bottom: none; }
        .vt-table-empty { padding: 24px 16px; color: var(--vt-text-muted); font-size: 12.5px; }

        .vt-footer { text-align: center; padding: 20px 0; font-size: 11.5px; color: #94a3b8; }
...
<body>
    ...
    {{-- Navbar admin --}}
    @include('refont.partials.navbar_console')
    ...
    <footer class="vt-footer">
        With ❤️ by VOTIUM &bull; Administration &bull; Tout droit réservé &copy; {{ date('Y') }}
    </footer>
    ...
    {{-- Gestionnaire global AJAX (identique espace promoteur) --}}
    <script>
            ...
            $(document).on('submit', '.ajax-form', function (e) {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        const alertData = { type: 'success', message: response.message || 'Action réussie !' };
                        ...
                        if (xhr.status === 422) {
                            ...
                            $.each(errors, function (fieldName, messages) {
                                ...
                                if ($input.length > 0) {
                                    ...
                                    let errorMsg = `<div class="invalid-feedback d-block" style="color:#dc3545;font-size:11.5px;margin-top:5px;">${messages[0]}</div>`;
                                    if ($input.closest('.vt-input-wrap').length) $input.closest('.vt-input-wrap').after(errorMsg);
                                    else if ($input.closest('.input-group').length) $input.closest('.input-group').after(errorMsg);
                                    else $input.after(errorMsg);
                                }
                            });

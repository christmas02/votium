<?php

namespace App\Http\Controllers;

use App\Models\Candidat;
use App\Repository\CandidatRepository;
use App\Sdkpayment\Hyperfast\HyperfastWebhook;
use App\Sdkpayment\Hub2\Hub2Webhook;
use App\Services\GeneratePdf;
use App\Services\Setting;
use App\Services\VoteService;
use Carbon\Carbon;
use Illuminate\Http\Request;

class WebhookController
{
    protected $hyperfastWebhook;
    protected $hub2Webhook;
    protected $voteService;
    protected $setting;
    protected $candidat;

    public function __construct(HyperfastWebhook $hyperfastWebhook, VoteService $voteService, Setting $setting,
                                CandidatRepository $candidat, Hub2Webhook $hub2Webhook)
    {
        $this->hyperfastWebhook = $hyperfastWebhook;
        $this->voteService = $voteService;
        $this->setting = $setting;
        $this->candidat = $candidat;
        $this->hub2Webhook = $hub2Webhook;

    }
    public function handleWebhookHyperfast(Request $request)
    {
        try {
            $response = [
                'id' => $request->id,
                'phone' => $request->phone,
                'carrier' => $request->carrier,
                'amount' => $request->amount,
                'currency' => $request->currency,
                'status' => $request->status,
                'carrier_transaction_id' => $request->carrier_transaction_id,
                'response' => $request->response,
                'syntax' => $request->syntax,
                'metadata' => $request->metadata,
                'created_at' => $request->created_at,
                'processed_at' => $request->processed_at,
            ];
            // Mise a jour de la transaction
            $transaction = $this->hyperfastWebhook->handleWebhook($response);
            logger()->info('Webhook Hyperfast traité pour la transaction ID ' . $transaction['transaction_id']);
            // Mise a jour de la vote
            $paramVote = [
                'vote_id' => $transaction['vote_id'],
                'status' => $transaction['status'],
            ];
            $vote = $this->voteService->updateVoteStatusAfterPayment($paramVote);
            // si la transaction est succes generer un recu de paiement
            if ($vote->status === 'confirmed') {
                $this->voteService->createInvoice($vote, $transaction);
            }
            \Log::info('webhookController handleWebhookHyperfast : ' . 'Webhook traité avec succès pour la transaction ID ' . $transaction['transaction_id']);

            return 'operation effectuere avec succes';

        } catch (\Exception $e) {
            \Log::error('Erreur webhookController handleWebhookHyperfast : ' . $e->getMessage());
            throw $e; // Rejeter l'exception pour gestion en amont
        }
    }

    public function handleWebhookHub2(Request $request)
    {
        try {
            $datas = $request->all();
            // Mise a jour de la transaction
            $transaction = $this->hub2Webhook->handleWebhook($datas);
            logger()->info('Webhook Hub2 traité pour la transaction ID ' . $transaction['transaction_id']);
            // Mise a jour de la vote
            $paramVote = [
                'vote_id' => $transaction['vote_id'],
                'status' => $transaction['status'],
            ];
            $vote = $this->voteService->updateVoteStatusAfterPayment($paramVote);
            // si la transaction est succes generer un recu de paiement
            if ($vote->status === 'confirmed') {
                //$this->generatePdf($vote, $transaction);
                $this->voteService->createInvoice($vote, $transaction);
            }
            \Log::info('webhookController handleWebhookHub2: ' . 'Webhook traité avec succès pour la transaction ID ' . $transaction['transaction_id']);
            return 'operation effectuere avec succes';
            
        } catch (\Exception $e) {
            \Log::error('Erreur webhookController handleWebhookHub2 : ' . $e->getMessage());
            throw $e; // Rejeter l'exception pour gestion en amont
        }
    }
}